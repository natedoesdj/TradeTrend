...
{
    internal class Recognizer_Engulfing_Bearish : Recognizer
    {
        public Recognizer_Engulfing_Bearish() : base("Engulfing(Bearish)", 2) { }

        public override bool Recognize(List<SmartCandlestick> sc, int index)
        {
            if (index < sc.Count - 1)
            {
                // Checks if the first candlestick is bullish
                bool result1 = sc[index].close > sc[index].open;

                // Checks if the second candlestick is bearish
